...
class EarlyStopping:
    ...
    def save_checkpoint(self, val_loss, model):
        ...
        torch.save(model.state_dict(), 'checkpoint_tf_v8.pt')
        self.val_loss_min = val_loss

class Transformer(torch.nn.Module):
    def __init__(self):
        super(Transformer, self).__init__()
        self.encoder_layer = torch.nn.TransformerEncoderLayer(d_model=36, nhead=2)
        self.transformer_encoder = torch.nn.TransformerEncoder(self.encoder_layer, num_layers=6)
...
if __name__ == '__main__':
    ...
    opt = torch.optim.Adam(model.parameters(),lr=2*1e-2)
    ...
    for epoch in range(100):
        ...
        if early_stopping.early_stop:
            ...
            break
    torch.save(model.state_dict(), 'tf_v8.pt')
